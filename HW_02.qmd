...
vectorNFIMissing = dfNRISubset.isnull().sum()

# And now create dataframe from vectorNFIMissing.
dfMissing = vectorNFIMissing.reset_index()
dfMissing.columns = ['Column', 'MissingCount']

# Find the hazard types and put them into a new col.
dfMissing['HazardType'] = dfMissing['Column'].str.extract(r'^(.*?)(?:_AFREQ|_RISKR)$')[0]

# Group, and sum the missing counts.
dfMissingCrossTabs = dfMissing.groupby('HazardType').sum()
print(dfMissingCrossTabs)
```
